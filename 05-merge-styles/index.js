...
fs.promises.readdir(srcFolder).then((files, err) => {
  ...
  return fs.promises.writeFile(path.join(__dirname, 'project-dist', 'bundle.css'), '');
}).then(() => {
  for(let i = 0; i < filesPaths.length; i++) {
    const readableStream = fs.createReadStream(filesPaths[i], 'utf-8');
    dataArray.push();
    readableStream.on('data', chunk => dataArray[i] += chunk.toString());
    readableStream.on('end', () => fs.promises.writeFile(path.join(__dirname, 'project-dist', 'bundle.css'), dataArray.join('')));
  };
});